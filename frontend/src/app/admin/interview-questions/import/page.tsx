...
import { importAPI, tagAPI } from '@/lib/api';
...
import type { Category as InterviewCategory } from '@/types/interview';
...
export default function ImportPage() {
  ...
  const fetchCategoriesAndTags = async () => {
    try {
      const [categoriesResult, tagsResult] = await Promise.all([
        interviewCategoryApi.getCategories(),
        tagAPI?.getList ? tagAPI.getList() : Promise.resolve({ code: 200, data: [] })
      ]);
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await fetchCategoriesAndTags(); // 获取分类和标签数据
  ...
  const fileColumns: ColumnsType<ImportFileInfo> = [
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <Select
          placeholder="选择分类"
          value={record.selectedCategoryId || text}
          onChange={(value) => {
            const newFiles = [...files];
            newFiles[index].selectedCategoryId = value;
            setFiles(newFiles);
          }}
          style={{ width: '100%' }}
          size="small"
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const processedFiles = selectedFiles.map(file => {
        const { selectedCategoryId, ...fileData } = file;
        return {
          ...fileData,
          category: selectedCategoryId || file.category
        };
      });
        ...
        {currentStep === 1 && (
          ...
          >
            <Space direction="vertical" size="large" style={{ width: '100%' }}>
                    ...
                    <Select
                      ...
                      onChange={(value) => {
                        const selectedIndices = files
                          .map((f, index) => f.selected ? index : -1)
              ...
              <Alert
                message="分类设置说明"
                description="您可以为每个文件单独选择分类，或使用'批量设置分类'功能为所选文件统一设置。如果未设置，将使用文件中的分类信息或默认分类。"
                type="info"
                showIcon
                ...
                <Card title="基础设置" size="small">
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    <div>
                      ...
                      <Select
                        placeholder="选择默认分类（可选）"
                        allowClear
                        value={importConfig.defaultCategoryId}
                        onChange={(value) => setImportConfig({
                          ...importConfig,
                      ...
                      <Text type="secondary" style={{ fontSize: 12, display: 'block', marginTop: 4 }}>
                        如果文件中未指定分类且未在文件预览中选择分类，将使用此默认分类
                      </Text>
                    </div>
                  </Space>
                </Card>
